...
if [ ! -f $POSTGRES_DATA_DIR/PG_VERSION ]; then
    ...
    /usr/lib/postgresql/17/bin/initdb -U postgres --pwfile=$HOME/pwfile $POSTGRES_DATA_DIR
    rm $HOME/pwfile
    /usr/lib/postgresql/17/bin/pg_ctl -D $POSTGRES_DATA_DIR start
    psql --command "CREATE DATABASE seechange OWNER postgres"
    psql --command "CREATE EXTENSION q3c" seechange
